<?php
   ...
   mysql_select_db("rejestr",$polaczenie) or die (mysql_error());
    $n=mysql_num_rows(mysql_query("SELECT count(id_czytelnika) FROM czytelnik where nazwisko='$_POST[fraza]'",$polaczenie));
   //echo"$n";
   for($i=1;$i<=$n;$i++){
      $uczen=mysql_query("SELECT imie,nazwisko,klasa,haslo,id_czytelnika FROM czytelnik WHERE nazwisko='$_POST[fraza]'",$polaczenie);
      $liczba_znalezionych=mysql_num_rows($uczen);
      $dane_listy=mysql_fetch_row($uczen);
   echo"<center valign=\"middle\"><b>$dane_listy[0] $dane_listy[1]</b> $dane_listy[2]<form action=\"$PHP_SELF\" method=\"post\">
   ...
   $wywolanie_danych=mysql_query("Select imie,nazwisko,klasa
                                  From czytelnik
   ...
   $dane=mysql_fetch_row($wywolanie_danych);
   $godzina=time();
   echo"<center valign=\"middle\">$dane[0] $dane[1] z $dane[2] wpisany na liste o godzinie $godzina</center>";
